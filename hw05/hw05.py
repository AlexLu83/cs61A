...
def replace_leaf(t, old, new):
    ...
    def helper(i):
        if is_leaf(i):
            ...
            return [label(i)] + [helper(b) for b in branches(i)]
    return helper(t)
...
def balanced(m):
    ...
    if not (is_mobile(end(left(m))) and is_mobile(end(right(m)))):
        ...
        return left_torque == right_torque and balanced(end(left(m)))
    else:
        return left_torque == right_torque and balanced(end(right(m)))
...
def make_fib():
    ...
    n = 2
    def next_fib():
        nonlocal fib, fib_n_1, fib_n_2, n
        if n > 1:
            n -= 1
            return fib
        fib, fib_n_1, fib_n_2 = fib + fib_n_1, fib, fib_n_1
        return fib
...
def make_joint(withdraw, old_password, new_password):
    ...
    "*** YOUR CODE HERE ***"
    def with_draw(amount, code):
        if code == old_password or code == new_password:
            return withdraw(amount, old_password)
        return withdraw(amount, code)
    s = with_draw(0, old_password)
    if type(s) == str:
        return s
    return with_draw
...
def generate_paths(t, x):
    ...
    "*** YOUR CODE HERE ***"
    l = []
    def path_list(tre, sub=[]):
        nonlocal l
        sub.append(label(tre))
        if label(tre) == x:
            l.append(sub)
        if not is_leaf(tre):
            for b in branches(tre):
                temp = sub[:]
                path_list(b, temp)
    path_list(t)
    for i in l:
