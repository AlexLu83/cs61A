...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def bst_min(tre, min=t.label):
        if tre.label < min:
            return False
        if tre.is_leaf():
            return True
        if len(tre.branches) > 2:
            return False
        if len(tre.branches) == 1:
            return bst_min(tre.branches[0])
        left = tre.branches[0]
        right = tre.branches[1]
        if left.label > tre.label or right.label <= tre.label:
            return False
        return bst_min(left) and bst_min(right)

    def bst_max(tre, max=t.label):
        if tre.label > max:
            return False
        if tre.is_leaf():
            return True
        if len(tre.branches) > 2:
            return False
        if len(tre.branches) == 1:
            return bst_max(tre.branches[0])
        left = tre.branches[0]
        right = tre.branches[1]
        if left.label > tre.label or right.label <= tre.label:
            return False
        return bst_max(left) and bst_max(right)
    ...
    left = t.branches[0]
    right = t.branches[1]
    return bst_max(left) and bst_min(right)
# Q8
...
def in_order_traversal(t):
    ...
    def traversal(tre):
        if tre.is_leaf():
            lst.append(tre.label)
        else:
            traversal(tre.branches[0])
            lst.append(tre.label)
            traversal(tre.branches[1])
    lst = []
    if t.is_leaf():
        lst += t.label
    else:
