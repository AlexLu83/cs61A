...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        proce = scheme_eval(first, env)
        check_procedure(proce)
        if isinstance(proce, MacroProcedure):
            return scheme_eval(proce.apply_macro(rest, env), env)
        eval = lambda x: scheme_eval(x, env)
        rest = rest.map(eval)
        return scheme_apply(proce, rest, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        symbol = formals
        value = vals
        while not symbol == nil:
            new_frame.define(symbol.first, value.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        body = scheme_eval(expressions.second.first, env)
        env.define(target, body)
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        new_expr = Pair('lambda', Pair(target.second, expressions.second))
        body = scheme_eval(new_expr, env)
...
def do_and_form(expressions, env):
    ...
    if expressions.second == nil:
        current = scheme_eval(expressions.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    temp = bindings
    check_form(temp.first, 2, 2)
    formal_list = Pair(temp.first.first, nil)
    val_list = Pair(scheme_eval(temp.first.second.first, env), nil)
    helper = lambda par, val: Pair(val, par)
    temp = temp.second
    while not temp == nil:
        check_form(temp.first, 2, 2)
        formal_list = helper(formal_list, temp.first.first)
        val_list = helper(val_list, scheme_eval(temp.first.second.first, env))
        temp = temp.second
    check_formals(formal_list)
    return env.make_child_frame(formal_list, val_list)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        body = MacroProcedure(target.second, expressions.second, env)
        env.define(target.first, body)
        return target.first
        # END PROBLEM 10
    else:
