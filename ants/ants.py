        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.is_container and self.ant.can_contain(insect):
                    ...
                    self.ant = insect
                elif self.ant.is_container and isinstance(insect, QueenAnt) and insect.true_queen:
                    self.contain_ant(insect)
                else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount<= 0:
            n = 0
            i = len(self.place.bees)
            while n < i:
                if self.place.bees[n].armor <= 3:
                    self.place.bees[n].reduce_armor(self.damage)
                    n -= 1
                    i -= 1
                else:
                    self.place.bees[n].reduce_armor(self.damage)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        self.true_queen = self.true_queen_class
        if self.true_queen_class:
            QueenAnt.true_queen_class = False
        self.armor = armor
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            while not loc == None:
                behind_queen += [loc]
                loc = loc.exit
            for i in behind_queen:
                if i.ant:
                    if (not i.ant.is_container) and not i.ant.doubled_damage:
                        i.ant.damage = 2 * i.ant.damage
                        i.ant.doubled_damage = True
                    elif i.ant.is_container:
                        if i.ant.contained_ant:
                            if not i.ant.contained_ant.doubled_damage:
                                if not i.ant.doubled_damage:
                                    i.ant.damage = 2 * i.ant.damage
                                    i.ant.contained_ant.damage = 2 * i.ant.contained_ant.damage
                                    i.ant.doubled_damage = True
                                    i.ant.contained_ant.doubled_damage = True
                                else:
                                    i.ant.contained_ant.damage = 2 * i.ant.contained_ant.damage
                                    i.ant.contained_ant.doubled_damage = True
                            else:
...
def make_scare(action, bee):
    ...
    def transfer(dur):
        ...
        def scare(colony):
            ...
            bee.is_scared = True
            if duration > 0:
                if not bee.place.entrance == Hive:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    if not (effect == make_scare and bee.is_scared):
